...
- Enabled Splunk ingestion of **OWASP Juice Shop** Docker **json-file** logs (stdout/stderr) to support faster app level triage alongside Suricata network telemetry (added new stanza to `inputs.conf`)
- Created a dedicated **`docker`** index and verified ingestion

![Docker Logs in Splunk](assets/docker_juiceshop_ingestion_001.png)
